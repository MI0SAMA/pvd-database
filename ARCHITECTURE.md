...
Excel 表头在不同批次间有变化， 提供集中管理的列别名映射：
...
### 动态参数发现
 ...
 新增  — 从 information_schema 自动查询有数据的数值列，分析页不再硬编码参数列表。新增 DB 列自动出现在下拉菜单中。

### 工艺参数分类更新

| 分类 | 列 |
|------|----|
| 新增 |  (AlSc靶功率, 27条),  (AlN靶功率, 36条) |
| 修复 | P5  49/49 恢复 |
...
### 列名模糊匹配 (import_utils.py)

Excel 表头在不同批次间有变化，`import_utils.py` 提供集中管理的列别名映射：
- 制备温度 → P5 改为 制备温度(°C) → 自动匹配
- P4/P5 新增 AlSc(W)/电压、AlN(W)/电压 列 → DB 新增 alsc_power_w, aln_power_w
- 导入时未识别的列会打印警告

### 动态参数发现 (db.py)

`get_valid_numeric_params()` — 从 information_schema 自动查询有数据的数值列，分析页不再硬编码参数列表。新增 DB 列自动出现在下拉菜单中。

### 修复

- P5 substrate_temp_set: 0/49 → 49/49 恢复
- 新增 alsc_power_w (27 条), aln_power_w (36 条)
- 删除旧迁移遗留的 P1(21-40) 和 P2(41-60) 无效样品
